...
def get_img():
    ...
    id_photo = len(os.listdir(img_save))
    for image_path in img_list:
        id_photo += 1
        xml_path_save = os.path.join(xml_path, image_path.split('\\')[-1][:-4] + '.xml')
        imgs = []
        imgs.append(image_path)
        imgs.append(xml_path_save)
        imgs.append(id_photo)
        try:
            yield imgs
        except:
            break
...
def change_photo(image_path, xml_path_save, id_photo):
    ...
    # img = random_bright(img, p=0.5, lower=0.8, upper=1.1)  ###随机对比度和亮度
    # img, bboxes = random_crop(img, bboxes=bboxes, p=0.5)  ###随即裁切
    img, bboxes = letterbox(img, new_shape=(640, 640), bboxes=bboxes)[0:2]
    ...
    # img, bboxes = random_rot90_1(img, bboxes=bboxes, p=1)  ###随机逆时针旋转90°
    # img, bboxes = random_translate(img, np.array(bboxes), 1)          ###随机平移 ****最好别用****
    # img, bboxes = random_crop(img, bboxes=bboxes, p=0.5)               ###随即裁切

    # img, bboxes = random_swap(img, np.array(bboxes), 1)               ###随即通道转换
    # img, bboxes = random_saturation(img, bboxes=bboxes, p=0.5)  ###随即变换饱和度
    # img, bboxes = random_hue(img, bboxes=bboxes, p=0.5)  ###随机变换色度
    # img, bboxes = random_rot(img, bboxes=bboxes, angle=np.random.randint(0, 90, 1)[0])  ###任意旋转角度

    # 图像存储操作；xml修改存储操作
    newpt = os.path.join(img_save, '{:0>8d}'.format(id_photo))
    new_xml = os.path.join(xml_save, '{:0>8d}'.format(id_photo))
    # newpt = os.path.join(img_save, image_path.split('\\')[-1][:-4]) + '_random_vertical_flip'
    # new_xml = os.path.join(xml_save, image_path.split('\\')[-1][:-4]) + '_random_vertical_flip'
    img = np.array(img)
...
def loop(imgs):
    ...
    while True:
        try:
            with lock:
                image_path, xml_path_save, id_photo = next(imgs)
        except StopIteration:
            break
        try:
            change_photo(image_path, xml_path_save, id_photo)
        except:
            print("change %s false", image_path)
...
for ii in range(thread_num):
    t = threading.Thread(target=loop, name='LoopThread %s' % ii, args=(imgs,))
    t.start()
